...
{
    public interface IOfficeService : IGenericService<TblMdOffice, OfficeDto>
    {
        Task<IList<OfficeDto>> GetAll(BaseMdFilter filter);
        Task Insert(OfficeDto data);
        Task UpdateOffice(OfficeDto data);
        Task<List<string>> GetATVSV(string headerId);



    }
    public class OfficeService(AppDbContext dbContext, IMapper mapper) : GenericService<TblMdOffice, OfficeDto>(dbContext, mapper), IOfficeService
    {
        public override async Task<PagedResponseDto> Search(BaseFilter filter)
        {
            try
            {
                var query = _dbContext.TblMdOffice.AsQueryable();
                if (!string.IsNullOrWhiteSpace(filter.KeyWord))
                {
                    query = query.Where(x => x.Id.ToString().Contains(filter.KeyWord) || x.Name.Contains(filter.KeyWord) || x.ThuTruong.Contains(filter.KeyWord) || x.NguoiPhuTrach.Contains(filter.KeyWord));
                }
        ...
        public new async Task Delete(string id)
        {
            try
            {
                var Office = _dbContext.TblMdOffice.FirstOrDefault(x => x.Id == id);

                Office.IsActive = false;
                _dbContext.TblMdOffice.Update(Office);
        ...
        public async Task Insert(OfficeDto data)
        {
            try
            {
                ...
                var CHT = lstAccount.FirstOrDefault(x => x.UserName == data.ThuTruong);
                CHT.ChucVuId = "CHT";
                ...
                var Office = new TblMdOffice()
                {
                    Id = data.Id,
                    Name = data.Name,
                    PhoneNumber = data.PhoneNumber,
                    ThuTruong = data.ThuTruong,
                ...
                _dbContext.TblMdOffice.Add(Office);
        ...
        public async Task UpdateOffice(OfficeDto data)
        {
            try
            {
                var lstUpdateAccount = new List<TblAdAccount>();
        ...
        public async Task<IList<OfficeDto>> GetAll(BaseMdFilter filter)
        {
            try
            {
                var query = _dbContext.TblMdOffice.AsQueryable();
